...
| retarget | `retarget` `retarget-ik`（実 G1 23 関節角・end-effector 重み付け・`--conf-gate` 遮蔽ガード）`export-joints`（実機/シム SDK 向け関節角 CSV/JSON）`list-retargeters`（builtin / GMR）`demo-multi`（G1/H1/T1/Apollo） |
