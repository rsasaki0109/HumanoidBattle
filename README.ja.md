...
<img src="assets/readme/tournament.gif" width="560" alt="HumanoidBattle トーナメント決勝: Fourier N1 が backflip で Unitree G1 を下し優勝">
...
| retarget | `retarget` `retarget-ik`（実 G1 23 関節角・end-effector 重み付け・`--conf-gate` 遮蔽ガード）`export-joints`（実機/シム SDK 向け関節角＋任意で `--with-velocity` 速度 CSV/JSON）`list-retargeters`（builtin / GMR）`demo-multi`（G1/H1/H2/T1/Apollo/N1）`demo-battle`（⚔️ 1v1 実行品質バトル・metric 採点）`demo-tournament`（🏆 単欠トーナメント→チャンピオン）`demo-fight`（🥊 MuJoCo で 2 体ボクシング・ヒット採点） |
