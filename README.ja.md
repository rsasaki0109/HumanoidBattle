...
<sub>残った ZMP のはみ出しは前後 x（単眼で最も不確実な深度）方向に偏る。完全 PASS には深度推定 / contact-aware retarget の改善が要る — v0 の正直な frontier。`--balance-refine` はその第一歩: 観測できている横/高さ（y, z）の画像面を凍結し、**未観測の前後 x 深度だけ**を quasi-static balance prior（COM を支持多角形上へ）で精緻化する — violation を隠す平滑ではなく精緻化。</sub>
...
| 領域 | 主なコマンド |
| --- | --- |
| 抽出 | `extract`（`--backend`）`import-hmr` `import-humanml3d` `import-babel` `import-motionx` `download-hf`（HF Hub 取得 → import-*、YouTube/TikTok の license-safe 代替）`smooth` `overlay` |
