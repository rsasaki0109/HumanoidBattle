...
- [x] **model cards v0**（`model-card`, `robotdance_core.model_card`）: RD-MIR/RD-Motion/**RD-Policy**
      から **data lineage・license・failure modes（手法 registry から検出）・safety limits**（policy は
      **I/O Contract・Weights** も）を Markdown + 機械可読 JSON で生成。種別自動判別。純 Python・CI 検証
