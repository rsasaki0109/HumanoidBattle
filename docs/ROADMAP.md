...
- [x] **RD-MIR semantics 構造化**（`robotdance_core.semantics`）: action_label / style_tag / captions /
      segments（連続行動 [{label,start_t,end_t}]）を spec 化。後方互換維持。BABEL が segments を出力
...
- [x] local video pipeline: **MediaPipe Pose** で decode → detect → world landmarks → canonical RD-MIR
      + **temporal smoothing**（Savitzky-Golay, jitter 指標）。
- [x] **HMR adapter v0**（`import-hmr`, `robotdance_perception.hmr`）: 4DHumans（HMR2.0/PHALP, rotmat）/
      GVHMR（axis-angle・world-grounded）の per-frame SMPL 出力 → skeleton-first FK で canonical RD-MIR。
      `from_gvhmr` / `from_4dhumans` / `load_hmr_file`（.npz/.npy/.pkl/.pt 自動判別）。**betas で
      shape-conditioning**（身長/体幅の粗いプロキシ, 真の blend shapes は今後）。weight 非同梱・CI 検証。
      multi-person tracking は今後
- [x] seed dataset adapters: **AMASS / AIST++ / HumanML3D / BABEL / Motion-X（skeleton-first, SMPL FK）** ✅
      （`import-humanml3d` / `import-babel` / `import-motionx`, text/行動ラベルを semantics に格納）
...
- [x] 重複除去: **motion embedding による near-duplicate 検出**を dataset build に統合（`--dedupe`）+
      **任意の RD-MIR コレクション（text-motion 含む）へ汎用化**（`robotdance_motion.dedupe.dedupe_mirs`,
      `import-babel --dedupe`, `dedupe-dir`）✅
...
- [x] simulation backend: **MuJoCo** で sim_certificate を埋める（逆動力学トルク + COM/ZMP バランス + 滞空）
      → safe dance=PASS / backflip=REJECT（§6.2 Demo 4, `demo-safety`）。
- [x] **sim backend 抽象 + registry v0**（`robotdance_sim.backend`, `sim-backends`,
      `validate-sim --backend`）: `SimBackend` 契約 + registry で backend を pluggable に。MuJoCo を
      参照実装、Isaac Lab を contract のみの scaffold として登録（本体は同梱しない）。Isaac Lab/Genesis
      の実装・GPU 並列 sim は今後
...
- [x] **実 URDF 取り込み**（`import-urdf`）: zero-config FK で実リンク寸法から RobotMorphology を構築
      （Unitree G1 23dof で nominal_height≈1.29m）。実機慣性は今後
- [x] **G1/H1 morphology を実 URDF 実寸に修正**（real-data validation）: 旧手書きプロキシは実機と乖離
      （G1 nominal 1.12m・bone 誤差 ~26% / H1 bone 誤差 ~33%）。公式 g1_23dof / h1.urdf 由来の実寸へ更新
      （誤差 ~0%）。実 G1 URDF で actuator IK 収束（mean err 0.062m, limit 違反 0）も確認。H1 用 `H1_LINK_MAP`
      追加（wrist 無 URDF は前腕合成）。`tests/test_real_g1_urdf.py` で G1/H1 とも回帰検証
- [x] retarget/sim benchmark + **leaderboard**（`benchmark`）: PASS率・bone_cos・foot_sliding・balance・torque を
      motion × robot で CSV/Markdown 集計（docs/benchmark/）。
- [x] **extraction benchmark v0**（`benchmark-extraction`, `robotdance_benchmarks.extraction`）: 抽出 adapter
      （MediaPipe/HMR）を共通 GT に対し MPJPE / PA-MPJPE / PCK / MPJVE / jitter / bone-length MAE で比較。
      純 numpy・CI 検証。実動画 GT スイートでの実モデル比較・leaderboard 提出フローは今後
...
- [x] **text-conditioned 生成 v0**（`train-text2motion` / `generate-text`）: token prior をテキスト特徴で
      ...
      語彙・新規 caption 汎化は今後
- [x] **RL tracking policy baseline v0**（`train-tracking` / `demo-track`,
      `robotdance_sim.tracking_env` + `robotdance_models.tracking_policy`）: MuJoCo forward 物理上で
      参照運動を倒れずに追従する方策を小型 **PPO** で学習（base 非駆動 = underactuated, 関節 PD + 残差トルク）。
      物理ロールアウトを RD-Motion（`control_mode="policy"`）で返す → 既存 pipeline に接続。
      合成 gentle 参照で survival 100%。PD 超えの精度・AMP/摂動/実機転移は今後
- [x] **multi-motion tracking v0**（`train-tracking --suite` / `demo-track-multi`,
      `robotdance_sim.MultiTrackingEnv`）: 参照スイート（gentle/normal/fast dance + idle）を round-robin で
      切替え **1 方策が複数運動を追従**（reference-conditioned 観測）。合成 4 運動で全運動 survival 100%。
      実 motion 規模での汎化・PD 超え精度は今後
- [ ] motion foundation model（denoising / 長尺）、高度な RL tracking（AMP/ASE, 摂動頑健性, 実機転移）
...
- [x] **viewer 強化 v0**（`render_gif` caption overlay / `render_search_montage` / `search-text --gif`）:
      RD-MIR の caption をオーバーレイし、text 検索 top-k を類似度バッジ付きモンタージュ GIF に描画。
      web インタラクティブ retrieval UI（upload → extract → similar 検索 → motion map）は今後
- [x] **motion foundation model baseline v0**: VQ-VAE + token prior で **completion / 生成 / 長尺生成
      （sliding-window）** + **双方向 denoiser（masked modeling）でノイズ除去・in-betweening** ✅
      （`train-denoiser` / `demo-denoise` / `demo-generate --length`）。長尺学習・diffusion は今後
- [x] **RL tracking baseline v0** ✅（G1/H1 tracking task, PPO 残差 on 関節 PD, base 非駆動）
      — AMP/ASE-style prior・摂動頑健性・実機転移は今後
- [x] **RD-Policy spec v0 確定 + policy export**（`export-policy`, `validate policy`,
      `robotdance_core.rd_policy`）: policy 配布 artifact（.rdpolicy）の schema/モデルを確定し、
      tracking policy → I/O 規約 + 学習来歴 + 安全制約 + weights 参照（+ONNX）を export。CI 検証
- [x] **model cards v0**（`model-card` / `cards-index`, `robotdance_core.model_card`）: RD-MIR/RD-Motion/
      **RD-Policy** から **data lineage・license・failure modes（手法 registry から検出）・safety limits**
      （policy は **I/O Contract・Weights** も）を Markdown + 機械可読 JSON で生成。種別自動判別。
      `cards-index` で dir 内 artifact を一括カード化し索引（CARDS_INDEX.md, license composition 付き）を
      出力。CITATION.cff 充実。純 Python・CI 検証
...
- [x] **joint-space safety guard v0**（`demo-joint-safety`, `clamp_joint_trajectory`）: 関節角列を
      実機コマンド直前に **位置・速度・加速度・トルク**へクランプ（`sim_certificate` の先の最終 gate,
      純 numpy・CI 検証）。位置/速度/トルクを bound・加速度は best-effort。トルクは実効慣性モデル
      （必要トルク≈I_eff·θ̈+重力）の計画段階 guard。電流飽和はモータ制御器（τ/Kt）が担う
- [x] **end-to-end pipeline ショーケース v0**（`demo-pipeline`, `robotdance_core.pipeline`）: 主要スタックを
      1 コマンドで繋ぐ（data/synth → RD-MIR → retarget → sim_certificate → [tracking policy + RD-Policy/
      ONNX export] → Model/Policy Card）。重い段は依存が無ければ graceful skip、core 段は CI 検証
...
- [x] **safety guard v0** ✅（Cartesian + **joint/velocity/accel/torque guard**, fall detector,
      E-stop, sim certificate check）— トルクは実効慣性モデルの計画段階 guard。電流飽和（τ/Kt）は
      モータ制御器、剛体動力学ベースの精緻なトルク推定は今後
