...
- [x] **model cards v0**（`model-card`, `robotdance_core.model_card`）: RD-MIR/RD-Motion から
      **data lineage（source→extractor→retarget→sim→policy）・license composition・failure modes
      （手法 registry から検出）・safety limits** を Markdown + 機械可読 JSON で生成。純 Python・CI 検証
