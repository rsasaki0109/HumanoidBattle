...
- [ ] core CLI: `validate` ✅ 雛形 / `build` / `extract` / `export` / `visualize` / `score`
- [ ] local video pipeline: decode → detection/tracking → 2D pose → 3D/HMR → smoothing → RD-MIR export
- [ ] viewer: original video / 2D overlay / 3D skeleton / root trajectory / contacts / quality timeline
...
- [ ] RD-Embodiment: G1 / H1 config（joint mapping, limits, contact surface, nominal pose）
- [ ] retargeting engine: contact-preserving IK, root trajectory adaptation, arm swing 保持, joint limit optimizer
- [ ] `.rdmotion` artifact + `rd-motion.schema.json` 確定
- [ ] simulation backend（まず 1 つ。推奨 Isaac Lab、MuJoCo は adapter で将来追加）
- [ ] retarget benchmark: foot slip, fall rate, tracking error, torque saturation proxy

**Acceptance:** 1 つの RD-MIR から G1 `.rdmotion` を生成し sim 再生。human/robot を side-by-side。safe/rejected を report。
