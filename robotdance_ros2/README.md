...
| `safety_guard.py` | **Safety Guard（§5.6）**: certificate gate, Cartesian 速度クランプ, **joint 空間 位置/速度/加速度/トルククランプ**, 転倒検知, E-stop, speed scaling | なし |
...
実機コマンド直前に **位置 limit・速度・加速度・トルク**へクランプする。`sim_certificate`（物理的妥当性）の
...
- `SafetyLimits` に `max_joint_speed` / `max_joint_accel` / `joint_position_limits` /
  `max_joint_torque` / `joint_inertia`（実効慣性）/ `joint_torque_limits` を設定
- **トルク**: 必要トルク ≈ I_eff·θ̈ + 重力負荷 を見積もり、トルク上限から加速度上限
...
> **sim-first** — 実機 bridge（unitree_sdk2）は安全レビュー後に別途接続。位置/速度/トルクは bound、
> 加速度は best-effort（位置 clamp の減速で残りうる）。トルクは**粗い実効慣性モデルの計画段階 guard**
> であり、完全な剛体動力学でもモータ電流飽和の代替でもない（電流は τ/Kt でモータ制御器が担う）。
> ros2_control 連携・実機再生は今後。custom .msg の colcon 化も今後の課題。
