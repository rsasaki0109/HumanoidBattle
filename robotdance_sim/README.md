...
> ⚠️ **v0 注意:** 既定の慣性は capsule 近似（質量分布は実 URDF inertial 由来 v0.34）。実 URDF
> `<inertial>` テンソルは `get_morphology(name, real_inertia=True)` で装着できる（**PD 追従は実慣性で
> 退行しないと v0.51 で実証**・certificate のトルク評価を補正、実測 H1 -22%）。既定が capsule なのは
> PPO baseline 退行回避のため。出力は "physically-informed feasibility" であって**実機保証ではない**。
> Isaac Lab backend は今後。`pip install -e ".[sim]"` で mujoco を入れる。
>
